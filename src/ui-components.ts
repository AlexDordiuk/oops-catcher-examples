/**
 * UI Components - Reusable UI utilities for demos
 */
...
export class UIComponents {
  ...
  addCustomField(containerId: string): void {
  ...
  collectCustomFields(containerId: string): Record<string, any> {
    const container = document.getElementById(containerId)
    if (!container) return {}
    ...
    rows.forEach(row => {
      const keyInput = row.querySelector('.custom-key') as HTMLInputElement
      const valueInput = row.querySelector('.custom-value') as HTMLInputElement
